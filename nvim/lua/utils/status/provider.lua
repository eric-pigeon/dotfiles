...
local env = require "utils.status.env"
local status_utils = require "utils.status.utils"
...
function M.foldcolumn(opts)
  opts = extend_tbl({ escape = false }, opts)
  local ffi = require "utils.ffi" -- get AstroNvim C extensions
...
function M.lsp_progress(opts)
  ...
  return function()
    local _, Lsp = next(vim.lsp.progress)
    return status_utils.stylize(Lsp and (spinner[math.floor(vim.uv.hrtime() / 12e7) % #spinner + 1] .. table.concat({
      Lsp.title or "",
      Lsp.message or "",
